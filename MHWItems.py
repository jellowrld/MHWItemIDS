...
def get_icon_path():
    """Returns the correct path to the MHW.ico icon file."""
    if getattr(sys, 'frozen', False):  # If running as an .exe
        return os.path.join(sys._MEIPASS, "MHW.ico")
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "MHW.ico")
...
if __name__ == "__main__":
    ...
    app.setWindowIcon(QIcon(ICON_PATH))  # Ensures taskbar icon is set
